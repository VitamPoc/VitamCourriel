...
public class Commands {
	...
	public static Element showFormat(String basename, String mimeCode, String format,
		...
		try {
			...
			if (config.droidHandler != null) {
				newElt.addAttribute("Droid", "6.1.3");
			}
			if (config.fits != null) {
				newElt.addAttribute("Fits", Fits.VERSION);
			}
			if (config.exif != null) {
				newElt.addAttribute("ExifTool", config.exif.getToolInfo().version);
			}
	...
	public static final void addFormatIdentification(Element root, String basename, File fic, 
		...
		try {
			...
			// XXX FIXME
			if (config.LOGDEBUG) {
				System.out.println("Start for file "+fic);
			}
			...
			// XXX FIXME
			if (config.LOGDEBUG) {
				System.out.println("Start FITS for file "+fic);
			}

			// Fits
			if (config.fits != null) {
				FitsOutput result = config.fits.examine(fic);
				org.jdom.Document docJdom = result.getFitsXml();
				org.w3c.dom.Document dom = output.output(docJdom);
				
				if (config.LOGDEBUG) {
					XMLOutputter serializer = new XMLOutputter(Format.getPrettyFormat());
					serializer.output(docJdom, System.err);
				}
				...
				if (element != null) {
					...
					if (sublist != null && !sublist.isEmpty()) {
						...
						for (int i = rk; i < nb ; i++) {
							toAdd = sublist.get(i);
							toAdd = (Element) toAdd.detach();
							Element newElt = XmlDom.factory.createElement("Fits");
							newElt.add(toAdd);
							toAdd = null;
							identification.add(newElt);
						}
					} else {
						toAdd = (Element) element.detach();
						Element newElt = XmlDom.factory.createElement("Fits");
						newElt.add(toAdd);
						toAdd = null;
						identification.add(newElt);
				...
				XmlDom.removeEmptyDocument(doc);
				Element newElt = XmlDom.factory.createElement("Fits");
				Iterator<Node> nodes = doc.getRootElement().nodeIterator();
			...
			if (config.exif != null) {
				if (config.LOGDEBUG) {
					System.out.println("Start EXIF for file "+fic);
				}
			...
			if (config.jhove != null) {
				if (config.LOGDEBUG) {
					System.out.println("Start JHOVE for file "+fic);
				}
			...
			if (argument.extractKeyword && finalFormat != null) {
				if (config.LOGDEBUG) {
					System.out.println("Start EXTRACTKW for "+basename+" file "+fic);
				}
			...
			root.addAttribute("status", status);
			if (config.LOGDEBUG) {
				System.out.println("End for file "+fic);
			}
		} catch (Exception e) {
			throw e;
		}
